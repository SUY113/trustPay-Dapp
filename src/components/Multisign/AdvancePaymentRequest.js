...
function AdvancePaymentRequest() {
  ...
  const handleEvaluateRequest = async () => {
    try {
      const res = await axios.post("http://localhost:3000/evaluate-request", {
        userName: userName,
        orgName: orgName,
        requestID: requestID,
      });
  ...
  const handleFinalizeRequest = async () => {
    try {
      const res = await axios.post("http://localhost:3000/finalize-request", {
        userName: userName,
        orgName: orgName,
        requestID: requestID,
      });
      ...
      if (res.data.result == "Request approved") {
        localStorage.setItem(
          "AccountIDToTransfer",
          localStorage.getItem("clientAccountIdReceive")
        );
        localStorage.setItem(
          "AmountToTransfer",
          localStorage.getItem("advanceToken")
        );
  ...
  return (
    <div className="request-form-container">
      ...
      <div>
        <button
          className="evaluate-request-button"
          onClick={handleEvaluateRequest}
        >
          Evaluate Request
        </button>
        <button
          className="finalize-request-button"
      ...
      <div>
        <button
          className="detail-advande-payment"
          onClick={handleShowRequestInfo}
        >
          Thong tin yeu cau ung tien
        </button>
      </div>
      {showRequestInfo && (
        <div className="request-info">
          ...
          <p>
            Client Account ID: {localStorage.getItem("clientAccountIdReceive")}
          </p>
